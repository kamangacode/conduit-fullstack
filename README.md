...
- **Bounded contexts** : un sous-dossier de domaine par concept RealWorld ([`user`](apps/api/src/domain/user), [`article`](apps/api/src/domain/article), [`comment`](apps/api/src/domain/comment), [`profile`](apps/api/src/domain/profile)). `tag` n'en est **pas** un et n'a plus de dossier de domaine : il n'a ni agrégat, ni invariant, ni entité, seulement un port de lecture qui vit en `application/` ([ADR 031](docs/adr/031-le-contrat-partage-s-arrete-a-la-frontiere-http.md)).
...
| **I**SP, ségrégation des interfaces | Lecture et écriture d'un article sont deux ports distincts, et ils ne vivent pas dans la même couche : [`article-repository.port.ts`](apps/api/src/domain/article/ports/article-repository.port.ts) (écriture, dans le domaine, il protège un agrégat) et [`article-query.port.ts`](apps/api/src/application/article/ports/article-query.port.ts) (lecture, applicatif, il sert un affichage). | [ISP](https://www.kamanga.fr/fr/architecture-craft/principe-isp-software-craftsmanship-exemples-java) |
