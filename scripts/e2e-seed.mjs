...
/**
 * Compte propriétaire du jeu de données.
 *
 * Un nom qui **s'annonce** comme un artefact de harnais : il apparaîtra sur la
 * page d'accueil pendant les runs, et quelqu'un finira par se demander d'où il
 * sort. Le mot de passe n'est pas un secret — il ouvre un compte d'une base
 * jetable, recréée à chaque run — mais il reste écrit ici plutôt que dans une
 * variable d'environnement pour que le script soit reproductible sans montage.
 */
const SEED_USER = {
...
/**
 * Article publié par ce compte.
 *
 * Un seul, et volontairement fade. Ce n'est pas une fixture applicative : plus
 * le jeu de données grossit, plus il devient une dépendance cachée des autres
 * fichiers de specs — et plus l'ordre d'exécution d'une suite parallélisée
 * finit par compter.
 */
const SEED_ARTICLE = {
...
async function resolveToken(baseUrl) {
  ...
  if (registered.status !== 422 && registered.status !== 409) {
    throw new Error(
      `inscription du compte de seed refusée (HTTP ${registered.status}) : ${registered.text}`
    )
  }
  ...
  const loggedIn = await call(baseUrl, {
    method: 'POST',
    path: '/users/login',
    body: { user: { email: SEED_USER.email, password: SEED_USER.password } },
  })

  if (!loggedIn.ok) {
    throw new Error(
      `connexion du compte de seed refusée (HTTP ${loggedIn.status}) : ${loggedIn.text}`
    )
  }
 ...
 */
async function seedArticleExists(baseUrl) {
  const listed = await call(baseUrl, {
    method: 'GET',
    path: `/articles?author=${encodeURIComponent(SEED_USER.username)}&limit=1`,
  })

  if (!listed.ok) {
    throw new Error(`lecture des articles du compte de seed échouée (HTTP ${listed.status})`)
  }
...
async function seed(baseUrl) {
  ...
  if (!token) {
    throw new Error("l'API n'a pas rendu de jeton pour le compte de seed")
  }

  if (await seedArticleExists(baseUrl)) {
    console.log('ok: jeu de données e2e déjà présent — rien à créer.')
    return
  }
  ...
  const created = await call(baseUrl, {
    ...
    path: '/articles',
    token,
    body: { article: SEED_ARTICLE },
  })

  if (!created.ok) {
    throw new Error(
      `création de l'article de seed refusée (HTTP ${created.status}) : ${created.text}`
    )
  }

  console.log(`ok: article de seed créé (${created.body?.article?.slug ?? 'slug inconnu'}).`)
}
